<template>
  <div class="page-wrapper">
    ...
    >
      ...
      <div class="back-button-container ga-3 mt-4 mb-5">
        <v-btn
      ...
      <v-card-title
        class="course-title font-weight-bold pt-0 pl-0"
        style="color: #333132"
      >
...
onMounted(() => {
  ...
  const savedCourseImage = localStorage.getItem(
    `original_course_image_${courseId.value}`,
  );
  if (savedCourseImage) {
...
const navigateToLesson = (lessonId) => {
  router.push(
    `/course/${courseId.value}/blocks/${blocksId.value}/lessons/${lessonId}`,
  );
};
