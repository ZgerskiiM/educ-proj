        ...
        <template v-else>
          <LessonCard
            ...
            :course-data="{
              ...
              imagePath: fixImageUrl(courseImage || blockImage), // Используем общее изображение
            }"
...
const blocksId = computed(() => route.params.keysId)

const courseImage = ref<string | null>(null) // Для хранения изображения курса
const blockImage = ref<string | null>(null) // Для хранения изображения блока
...
const fetchCourseData = async (id) => {
  ...
  try {
    ...
    courseBlocks.value = response.keys || []
...
const fetchBlockData = async () => {
  ...
  try {
    ...
    blockImage.value = data.imageUrl || null // Предполагаем, что imageUrl блока приходит в ответе
  } catch (err) {
...
// Обновленная функция навигации к уроку
